...
export async function POST(request: NextRequest) {
  try {
    ...
    const { userId, questionId, userAnswer, wordCount, evaluation } = body;
    ...
    const writingRecord = await prisma.dailyStudyRecord.create({
      data: {
    ...
    console.error('Failed to save writing record:', error);
    return NextResponse.json(
      { success: false, error: '保存写作记录失败' },
      { status: 500 }
    );
  }
}